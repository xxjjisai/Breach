GridClickSystem = class("GridClickSystem",System)

function GridClickSystem:start()
    self.bMoved = nil;
    self.tbEndPoint = nil;
end

function GridClickSystem:mousepressed(actors,x,y,button)
    if button ~= 1 then 
        return 
    end
...
function GridClickSystem:CheckCanWalk(nCol,nRow,tbRealMapInfo)
...
function GridClickSystem:GetNodeFromRealMap(nCol,nRow,tbRealMapInfo)
    for i,tbNode in ipairs(tbRealMapInfo) do 
        if nCol == tbNode.nCol and nRow == tbNode.nRow then 
            return tbNode;
        end
