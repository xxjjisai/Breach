A_BumpWorldSystem = class("A_BumpWorldSystem",System)
A_BumpWorldSystem.world = bump.newWorld(option.nWorldCellSize);
function A_BumpWorldSystem:start(actors)
...
function A_BumpWorldSystem:update(dt,actors)
    local actors = actors or {};
    if not next(actors) then 
        return 
    end
...
function A_BumpWorldSystem:draw(actors)
    if bDebug then 
        bump_debug.draw(self.world)
    end
end

function A_BumpWorldSystem:AddInWorld(actor)
    local compo_BumpWorld = actor:GetCompo("BumpWorld");
...
function A_BumpWorldSystem:ChangeInWorld(actor)
    local compo_Position = actor:GetCompo("Position");
    local compo_Size = actor:GetCompo("Size");
    local x = compo_Position:GetData("x");
    local y = compo_Position:GetData("y");
...
function A_BumpWorldSystem:MoveInWord(actor)
    local compo_Position = actor:GetCompo("Position");
    local x = compo_Position:GetData("x");
    local y = compo_Position:GetData("y");
...
function A_BumpWorldSystem:RemoveInWorld(actor)
