R_MapMakerSystem = class("R_MapMakerSystem",System)

function R_MapMakerSystem:start()
...
function R_MapMakerSystem:CreateMap(tbRealMapInfo)
...
function R_MapMakerSystem:GeneratorHandler(pfn)
    local iMap = actmgr:GMap();
    local iMapCompo = iMap:GetCompo("MapMaker").data;
...
function R_MapMakerSystem:GetSameItem(tbDataMapInfo,tbNode)
...
function R_MapMakerSystem:QuerytbNodeByColAndRow(iMapCompo,nCol,nRow)
...
function R_MapMakerSystem:GetRandomNode(iMapCompo,nType)
    for i = 1, #iMapCompo.tbRealMapInfo do
        local tbNode = iMapCompo.tbRealMapInfo[i];
...
function R_MapMakerSystem:GetNeighbors(iNode)
    return iNode.tbNeighors;
end

function R_MapMakerSystem:AddNeighbors(iNode,tbRealMapInfo)
    local x = iNode.x ;
    local y = iNode.y ;
...
function R_MapMakerSystem:draw()
    local iMap = actmgr:GMap();
    local iMapCompo = iMap:GetCompo("MapMaker").data;
